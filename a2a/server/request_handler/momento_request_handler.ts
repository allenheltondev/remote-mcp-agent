...
import { RequestContext } from "../agent/request_context.js";
...
import { MomentoTaskStore, TaskStore } from "../store.js";
import { A2ARequestHandler } from "./a2a_request_handler.js";

export interface MomentoRequestHandlerOptions {
    waitForAgentOnTaskCancellation?: boolean,
    momentoApiKey: string,
    cacheName: string;
}

export class MomentoRequestHandler implements A2ARequestHandler {
    ...
    constructor(agentCard: AgentCard, agentExecutor: AgentExecutor, options: MomentoRequestHandlerOptions) {
        this.agentCard = agentCard;
        this.taskStore = new MomentoTaskStore(options.cacheName, options.momentoApiKey);
        this.agentExecutor = agentExecutor;
        this.waitForAgentOnTaskCancellation = options.waitForAgentOnTaskCancellation ?? true;
        this.eventBusManager = new ExecutionEventBusManager(options.cacheName, options.momentoApiKey);
    }
    ...
    private async _createRequestContext(incomingMessage: Message, isStream: boolean, resultManager: ResultManager): Promise<RequestContext> {
        ...
        return new RequestContext(messageForContext, cancellationChecker, task, referenceTasks);
    }
